...
- **Immediate target: Render** — `render.yaml` (Blueprint) prepared: a `smart-library` Docker web service + a `smart-library-db` Postgres instance, `healthCheckPath: /up`, `APP_KEY` platform-generated, all other secrets `sync: false` (entered once in the dashboard, never in the file). Intended flow: `GitHub → CI → Docker build → Render Web Service → Render PostgreSQL`. Full walkthrough, including the free-tier Pre-Deploy Command limitation and the safe manual alternative, in `docs/DEPLOYMENT.md`.
